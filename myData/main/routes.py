...
from myData.main.forms import EditProfile, PostForm
from flask_login import current_user, login_user, logout_user, login_required
from myData.models import User, Post
from werkzeug.urls import url_parse
from datetime import datetime
from myData.main import bp
...
def index():
    ...
    posts = current_user.followed_post().paginate(page, current_app.config['POST_PER_PAGE'], False)
    next_url = url_for('main.index', page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('main.index', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('index.html', posts=posts.items, form=form, title='Home page',
...
def user(username):
    ...
    prev_url = url_for('main.user', username=user.username, page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('user.html',user=user, title='Profile', posts=posts.items,
                           next_url=next_url, prev_url=prev_url)
...
def follow(username):
    ...
    if user is None:
        flash('this user {} does not exist!!!'.format(username))
        return redirect(url_for('main.index'))
